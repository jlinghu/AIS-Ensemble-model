...
if __name__ == '__main__':
    ...
    bertModel = BertModel.from_pretrained('/root/bert')
    
    setup_seed(42)
    dev_text = pd.read_csv('/root/250k_qed/250k_qed_dev.csv')['smiles'].values.tolist()
    dev_label = pd.read_csv('/root/250k_qed/250k_qed_dev.csv')['qed'].values.tolist()
    test_text = pd.read_csv('/root/250k_qed/250k_qed_test.csv')['smiles'].values.tolist()
    test_label = pd.read_csv('/root/250k_qed/250k_qed_test.csv')['qed'].values.tolist()
    train_batch_size = 16
    mse_loss = nn.MSELoss()
    ...
    best_model = torch.load('/root/ais_bert/bert_250k_qed_best_model.pt')
    ...
    pre_list_pd.to_csv('BERT_250k_qed_pre.csv',index=False)