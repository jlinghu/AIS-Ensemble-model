...
if __name__ == '__main__':
    ...
    setup_seed(42)
    dev_text = pd.read_csv('/home/ubuntu/250k_qed/250k_qed_dev.csv')['smiles'].values.tolist()
    dev_label = pd.read_csv('/home/ubuntu/250k_qed/250k_qed_dev.csv')['qed'].values.tolist()
    test_text = pd.read_csv('/home/ubuntu/250k_qed/250k_qed_test.csv')['smiles'].values.tolist()
    test_label = pd.read_csv('/home/ubuntu/250k_qed/250k_qed_test.csv')['qed'].values.tolist()
    train_batch_size = 16
    mse_loss = nn.MSELoss()
    lr = 0.00001
    ...
    best_model = torch.load('/home/ubuntu/ais_xlnet/xlnet_250k_qed_best_model.pt')
    ...
    with torch.no_grad():
        ...
        for bi, (batch_text, batch_label,batch_attention_mask) in tqdm(enumerate(dev_dataloader, start=1)):
            ...
            if len(batch_text) != 1:
                dev_r2 += r2_score(batch_label.detach().cpu().numpy(),true_pre.detach().cpu().numpy()) * len(batch_text)
    ...
    pre_list_pd.to_csv('XLNet_250k_qed_pre.csv',index=False)