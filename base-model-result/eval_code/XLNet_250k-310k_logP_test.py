from transformers import XLNetModel,BertTokenizer
import torch
...
if __name__ == '__main__':
    ...
    best_model = torch.load('/root/xlnet_310k_logP_best_model.pt')
    ...
    with torch.no_grad():
        ...
        for bi, (batch_text, batch_label,batch_attention_mask) in tqdm(enumerate(test_dataloader, start=1)):
            ...
            batch_attention_mask = batch_attention_mask.to(device)
            loss, pre = best_model.forward(batch_text, batch_label,batch_attention_mask)
            test_rmse += (mse_loss(pre, batch_label).detach().cpu().numpy() ** 0.5) * len(batch_text)
