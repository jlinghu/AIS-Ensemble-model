from transformers import BertModel,BertTokenizer,AutoModel,XLNetModel
...
def get_smiles():
    df = pd.read_csv('/root/310k_zinc.csv')
    label = []
    smiles = []
    for row in range(len(df)):
        smiles.append(df['SMILES'][row])
        label.append(df['LogP'][row])
    return smiles,label
...
if __name__ == '__main__':
    model_file = '/root/xlnet'
    tokenizer = BertTokenizer.from_pretrained('/root/bert1', use_fast=True)
    bertModel = XLNetModel.from_pretrained(model_file)
    
    setup_seed(42)
    text, label = get_smiles()
    ...
    best_model = torch.load('/root/xlnet-310k_logP_best_model(smiles).pt')
    ...
    pre_list_pd.to_csv('111.csv',index=False)