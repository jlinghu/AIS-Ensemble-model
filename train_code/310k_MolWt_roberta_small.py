...
if __name__ == '__main__':
    ...
    setup_seed(42)
    train_text = pd.read_csv('/root/UN_data/310k_MolWt/310k_MolWt_train.csv')['smiles'].values.tolist()
    train_label = pd.read_csv('/root/UN_data/310k_MolWt/310k_MolWt_train.csv')['MolWt'].values.tolist()
    dev_text = pd.read_csv('/root/UN_data/310k_MolWt/310k_MolWt_dev.csv')['smiles'].values.tolist()
    dev_label = pd.read_csv('/root/UN_data/310k_MolWt/310k_MolWt_dev.csv')['MolWt'].values.tolist()
    test_text = pd.read_csv('/root/UN_data/310k_MolWt/310k_MolWt_test.csv')['smiles'].values.tolist()
    test_label = pd.read_csv('/root/UN_data/310k_MolWt/310k_MolWt_test.csv')['MolWt'].values.tolist()
    train_batch_size = 16
    mse_loss = nn.MSELoss()
    trian_mae_list = []
    train_rmse_list = []
    ...
    for e in range(epoch):
        ...
        print(f'train_RMSE:{np.ssqrt(train_rmse/len(train_text))}')
        ...
        if e > 60 and loss_ < best_loss:
                best_loss = loss_
                torch.save(model, "roberta_310k_MolWt_best_model.pt")

    best_model = torch.load('roberta_310k_MolWt_best_model.pt')
    ...
    result_data.to_csv('roberta_310k_MolWt_result_data.csv',index=False)