...
from transformers import AutoTokenizer, AutoModel,XLNetModel
...
class asmilesModel(nn.Module):
    ...
    def forward(self,batch_text,batch_label,batch_attention_mask):
        bert_out0,_ = self.bert.forward(batch_text,attention_mask=batch_attention_mask,return_dict=False)
        # bertout0: batch * seq * 768
        bert_out0 = self.tanh(bert_out0)
...
if __name__ == '__main__':
    ...
    bertModel = AutoModel.from_pretrained('/root/roberta-small')
    setup_seed(42)
    train_text = pd.read_csv('/root/data/UN_data/310k_logP/310k_logP_train.csv')['smiles'].values.tolist()
    train_label = pd.read_csv('/root/data/UN_data/310k_logP/310k_logP_train.csv')['logP'].values.tolist()
    dev_text = pd.read_csv('/root/data/UN_data/310k_logP/310k_logP_dev.csv')['smiles'].values.tolist()
    dev_label = pd.read_csv('/root/data/UN_data/310k_logP/310k_logP_dev.csv')['logP'].values.tolist()
    test_text = pd.read_csv('/root/data/UN_data/310k_logP/310k_logP_test.csv')['smiles'].values.tolist()
    test_label = pd.read_csv('/root/data/UN_data/310k_logP/310k_logP_test.csv')['logP'].values.tolist()
    train_batch_size = 16
    mse_loss = nn.MSELoss()
    trian_mae_list = []
    train_rmse_list = []
    ...
    for e in range(epoch):
        ...
        if e > 60 and loss_ < best_loss:
                best_loss = loss_
                torch.save(model, "roberta_310k_logP_best_model(nosuoxiao).pt")

    best_model = torch.load('roberta_310k_logP_best_model(nosuoxiao).pt')
    ...
    result_data.to_csv('roberta_310k_logP_result_data(nosuoxiao).csv',index=False)