...
class asmilesModel(nn.Module):
    ...
    def forward(self,batch_text,batch_label,batch_attention_mask):
        ...
        # x = self.linear(x)
        loss = self.loss_fun(x.squeeze(1),batch_label/10)

        return loss*10,x.squeeze(1)*10
...
if __name__ == '__main__':
    ...
    setup_seed(42)
    train_text = pd.read_csv('/root/UN_data/250k_LogP/250k_logP_train.csv')['smiles'].values.tolist()
    train_label = pd.read_csv('/root/UN_data/250k_logP/250k_logP_train.csv')['logP'].values.tolist()
    dev_text = pd.read_csv('/root/UN_data/250k_logP/250k_logP_dev.csv')['smiles'].values.tolist()
    dev_label = pd.read_csv('/root/UN_data/250k_logP/250k_logP_dev.csv')['logP'].values.tolist()
    test_text = pd.read_csv('/root/UN_data/250k_logP/250k_logP_test.csv')['smiles'].values.tolist()
    test_label = pd.read_csv('/root/UN_data/250k_logP/250k_logP_test.csv')['logP'].values.tolist()
    train_batch_size = 16
    mse_loss = nn.MSELoss()
    trian_mae_list = []
    train_rmse_list = []
    ...
    for e in range(epoch):
        ...
        if e > 60 and loss_ < best_loss:
                best_loss = loss_
                torch.save(model, "roberta_250k_logP_best_model.pt")

    best_model = torch.load('roberta_250k_logP_best_model.pt')
    ...
    result_data.to_csv('roberta_250k_logP_result_data.csv',index=False)